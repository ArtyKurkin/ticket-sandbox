...
- Telegram-уведомления наставникам о новых событиях.
- Уведомление, когда попытка отправлена на ручную проверку.
- Уведомление, когда стажер технически прошел все доступные задания.
- Прогресс по очереди заданий.
- Бейдж «Ждут проверки» в dashboard наставника.
- Дашборд стажера.
...
- GitHub Actions workflow для staging deploy.
- Smoke-check staging через `/`, `/healthz/` и `/admin/login/`.
...
- `TELEGRAM_BOT_TOKEN` — токен Telegram-бота для уведомлений наставникам;
- `TELEGRAM_CHAT_ID` — chat id лички или группы наставников.

Если `TELEGRAM_BOT_TOKEN` или `TELEGRAM_CHAT_ID` не заданы, Telegram-уведомления считаются выключенными и не мешают работе тренажера.
...
```bash
make sync-check      # строгая dry-run синхронизация training_tasks
...
Для CI используется SQLite, чтобы проверки не требовали отдельного PostgreSQL-сервиса.

После успешных тестов push в `main` запускает deploy на staging.
...
systemd status
smoke-check homepage
smoke-check /healthz/
smoke-check /admin/login/
```

Для smoke-check используется GitHub secret:
...
python manage.py test sandbox.tests.test_technical_lock
```

Ключевые проверки Telegram-уведомлений и dashboard наставника:
...
На текущем этапе проект является учебным MVP, но уже достаточно стабилен для staging-пилота.
...
- `check.sh` ограничен timeout;
- базовые ошибки Docker API обрабатываются без 500 для пользователя;
...
- добавлен первый L1-пакет учебных заданий;
- добавлены Telegram-уведомления для наставников.

Перед использованием с живыми стажерами нужно дополнительно доработать:

- пройти полный staging checklist;
...
- усилить обработку отдельных ошибок Docker API;
...
- Добавить отдельные фоновые статусы для долгих операций запуска, проверки и перезапуска.
...
1. Пройти staging вручную по `STAGING_CHECKLIST.md`.
2. Проверить первый L1-пакет заданий на staging от лица стажера и наставника.
3. Добавить polling статуса проверки для более живого UX.
4. Доработать фоновые статусы запуска, проверки и перезапуска.
5. Расширить production-документацию по nginx, terminal gateway, cookies и TLS.
6. Улучшить аналитику по стажерам и заданиям.
...
Проверить и применить синхронизацию учебных заданий из `training_tasks`:
...
Собрать Docker-образы учебных заданий:
...
Образы учебных заданий собираются командой:

```bash
python manage.py build_task_images
```

В staging/CD эта команда выполняется после `sync_training_tasks --strict` и до `collectstatic`.
...
Если включены Telegram-уведомления, в `.env` должны быть заданы:
...
Если эти переменные пустые, уведомления просто не отправляются.
...
2. Открыть `/healthz/` и проверить JSON со статусом `ok`.
3. Войти стажером и открыть dashboard.
4. Открыть L1-задание.
5. Нажать «Начать работу».
6. Убедиться, что терминал открылся через `/terminal/<attempt_id>/<port>/`.
7. Выполнить техническую часть задания и запустить автопроверку.
8. Убедиться, что `check.sh` отработал и `technical_passed_at` заполнен.
9. Если задача требует `requires_manual_review=True`, заполнить ответ клиенту и внутренний комментарий после технической сдачи.
10. Проверить, что попытка попала в mentor dashboard и бейдж «Ждут проверки» увеличился.
11. Проверить ручную проверку наставником.
12. Если Telegram env настроен, проверить уведомление о новой ручной проверке.
13. Проверить, что после успешной технической сдачи обычный перезапуск заблокирован.
14. Проверить, что историческая попытка открывается только в read-only режиме.

### Staging checklist

Для ручной проверки staging после деплоя используется отдельный чеклист:

```text
STAGING_CHECKLIST.md
```

Он нужен, чтобы пройти полный пользовательский сценарий:

- вход стажера;
- dashboard;
- запуск задания;
- terminal gateway;
- автопроверку через `check.sh`;
- ручную проверку наставником;
- повторную тренировочную попытку;
- read-only режим исторической попытки.

Перед первым пилотом staging нужно пройти по этому чеклисту и зафиксировать найденные проблемы.
