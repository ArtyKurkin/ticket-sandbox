...
В этом же проекте находится `traineediary` — внутренний Дневник стажёра для наставников: канбан адаптации, недельные метрики, оценка готовности и завершение испытательного срока.
...
Приложение `traineediary` доступно по адресу `/diary/` только пользователям с `User.is_staff=True`.

Основные сущности:

- `TraineeStage` — этап адаптации;
- `TraineeJourney` — путь сотрудника по этапам;
- `WeeklyMetric` — недельные показатели скорости и качества;
- `CompletionStatus` — результат завершения испытательного срока.

Качество учитывается только на этапе работы с проверками. После перехода дальше сохраняется последнее актуальное значение.
...
├── traineediary/        # дневник стажёра и адаптация
├── static/              # CSS и JS
