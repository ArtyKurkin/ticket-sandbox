...
- [ ] Если у задачи `requires_manual_review=True`, после технической сдачи стажер видит форму ответа клиенту.
...
Важно: у `cleanup_task_containers` пока нет `--dry-run`, поэтому на staging запускай команду осознанно и после проверки текущих контейнеров через `docker ps -a`.
