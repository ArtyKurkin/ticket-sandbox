...
## Неделя 17 — Улучшение Ticket Sandbox по обратной связи

### Обратная связь

- Собраны первые отзывы сотрудников после прохождения Ticket Sandbox.
- Подтверждено, что тренажёр хорошо работает как переход от теории к реальным тикетам.
- Выделены основные направления улучшения:
  - увеличить поля для ответа клиенту и внутреннего комментария;
  - добавить поддержку `systemctl reload` в учебных окружениях;
  - синхронизировать время с МСК;
  - не закрывать терминал сразу после успешной технической проверки;
  - расширять банк практических кейсов, в том числе сетевыми сценариями и ТСПУ.

### Архитектурные решения

- Разделено понятие технической блокировки и доступа к терминалу:
  - `technical_locked` по-прежнему запрещает повторный `check.sh`, restart и изменение технической части;
  - терминал может оставаться доступным после успешной технической проверки.
- Для зачётных заданий с ручной проверкой принят новый flow:
  - стажёр исправляет проблему;
  - проходит техническую проверку;
  - окружение блокируется от restart и повторного check;
  - терминал остаётся доступен;
  - стажёр готовит ответ клиенту и внутренний комментарий;
  - после отправки ответа наставнику окружение удаляется.
- Для дополнительных попыток и заданий без ручной проверки сохранено прежнее поведение:
  - после успешной технической проверки окружение сразу удаляется;
  - попытка переводится в завершённое состояние.
- Дополнительную попытку нельзя создать между успешной технической проверкой и отправкой ответа наставнику.
- Удаление окружения при отправке ответа сделано best effort:
  - временная ошибка Docker API не должна блокировать отправку ответа наставнику;
  - после перехода в `ON_REVIEW` доступ к терминалу закрывается независимо от результата cleanup.
- Для учебных контейнеров выбран отдельный timezone `MSK-3`, чтобы время UTC+3 работало независимо от наличия `tzdata`.
- В Django для пользовательского интерфейса используется `Europe/Moscow`.
- Для зависших окружений после успешной технической проверки добавлен TTL 72 часа от `technical_passed_at`.

### Интерфейс

- Увеличены поля:
  - ответа клиенту;
  - внутреннего комментария / отчёта стажёра.
- Размеры изменены только для формы задания, без глобального увеличения всех `textarea` проекта.
- После успешной технической проверки терминал больше не исчезает из карточки задания.
- Добавлено пояснение, что:
  - техническая часть уже зафиксирована;
  - окружение нельзя перезапустить;
  - терминал останется доступен до отправки ответа наставнику.
- Кнопка повторного прохождения скрывается до полного завершения зачётной попытки.

### Учебные окружения

- Проверены обёртки `systemctl` во всех L1-кейсах.
- Поддержка `reload` уже присутствовала в:
  - `nginx-413-upload`;
  - `php-fpm-down`;
  - `wordpress-500-after-move`.
- Добавлен `systemctl reload` в:
  - `nginx_not_starting`;
  - `sajt-ne-rabotaet-posle-perenosa`.
- Для task-контейнеров добавлена передача timezone:
  - `TZ=MSK-3`.
- Веб-интерфейс Ticket Sandbox переведён на московский часовой пояс.

### Логика технической проверки

- После успешного `check.sh` для зачётной попытки с ручной проверкой:
  - устанавливается `technical_passed_at`;
  - `check_status` становится `PASSED`;
  - попытка остаётся в `IN_PROGRESS`;
  - `finished_at` не устанавливается;
  - task-контейнер и terminal-контейнер сохраняются;
  - терминал остаётся доступным.
- После успешного `check.sh` для задания без ручной проверки:
  - контейнеры удаляются;
  - попытка переводится в `PASSED`;
  - сохраняется прежняя автоматическая логика завершения.
- Добавлено отдельное правило доступа к терминалу через `can_access_terminal`.
- Добавлено отдельное правило доступности повторного прохождения через `can_rerun`.
- `terminal_auth` теперь проверяет доступ к терминалу по новому правилу, а не только по `technical_locked`.

### Cleanup окружения

- При отправке ответа наставнику:
  - удаляется terminal-контейнер;
  - удаляется task-контейнер;
  - очищаются ссылки и идентификаторы контейнеров;
  - состояние окружения возвращается в `IDLE`.
- После отправки ответа и перехода в `ON_REVIEW` терминал становится недоступен.
- Добавлен автоматический cleanup окружений, в которых:
  - техническая проверка уже успешно пройдена;
  - ответ наставнику ещё не отправлен;
  - с момента `technical_passed_at` прошло более 72 часов.
- При таком cleanup:
  - Docker-окружение удаляется;
  - `technical_passed_at` сохраняется;
  - `check_status=PASSED` сохраняется;
  - попытка остаётся `IN_PROGRESS`;
  - повторно проходить техническую проверку не требуется;
  - стажёр сохраняет возможность позже отправить текстовый ответ наставнику.
- Для окружений младше 72 часов терминал и контейнеры сохраняются.
- TTL считается от момента успешной технической проверки, а не от начала задания.

### Исправлено

- Убрана преждевременная очистка контейнеров до проверки типа попытки.
- Исправлена двойная очистка окружения в ветке успешной технической проверки.
- Закрыт edge case, при котором стажёр мог создать дополнительную попытку сразу после технического pass и оставить старый контейнер вне текущего flow.
- Актуализирован старый тест, который ожидал удаления контейнера после успешной технической проверки manual-review задания.

### Проверки

- Обновлены тесты сервиса технической проверки под новый жизненный цикл контейнера.
- Добавлены проверки, что после технического pass:
  - зачётная попытка остаётся `IN_PROGRESS`;
  - `technical_locked=True`;
  - `technical_passed_at` заполнен;
  - `finished_at` остаётся пустым;
  - task-контейнер сохраняется;
  - terminal-контейнер сохраняется;
  - терминал доступен;
  - restart окружения запрещён.
- Добавлен тест доступа через `terminal_auth` после успешной технической проверки.
- Добавлен тест запрета `rerun` до отправки ответа наставнику.
- Добавлен тест очистки окружения при отправке ответа наставнику.
- Добавлены тесты 72-часового cleanup:
  - старое окружение после технического pass удаляется;
  - результат технической проверки сохраняется;
  - свежее окружение младше 72 часов не удаляется.
- Targeted-наборы тестов после изменений проходят успешно.
- Полный набор `sandbox` после изменений проходит успешно.
- Изменения проверены и отправлены в репозиторий.

### Модуль оценки знаний

- Продолжена разработка отдельного модуля оценки знаний сотрудников внутри Ticket Sandbox.

- Сформирована и расширена техническая матрица L1 по направлениям:
...
- Закрыто первичное покрытие всех технических навыков L1 вопросами.

- После первичного покрытия начато добавление дополнительных вариантов диагностических ситуаций, чтобы сотрудники не получали одинаковые сценарии при повторных тестированиях.

- Зафиксирован принцип построения банка:

- тест выбирает навык;

- внутри навыка выбирается семейство диагностической ситуации;

- из семейства используется один вариант вопроса;

- разные вопросы одного семейства проверяют одну и ту же компетенцию на разных исходных данных.

### Банк вопросов оценки знаний

- Существенно расширен банк практических диагностических вопросов по Linux.

- Добавлены сценарии по:

- поиску процесса, создающего CPU-нагрузку;

- росту потребления RAM отдельным процессом;

- интерпретации `free`, `available`, swap и memory pressure;

- OOM Killer;

- исчерпанию inode при наличии свободного места на диске;

- расхождению `df` и `du`;

- открытым удалённым файлам после logrotate;

- расширению виртуального диска, раздела и файловой системы;

- высокому Load Average при дисковом I/O;

- высокому Load Average из-за процессов в `D-state` и проблем с NFS;

- ошибкам запуска systemd-сервисов;

- `Start request repeated too quickly` как следствию, а не первичной причине ошибки;

- сопоставлению systemd-сервиса, PID и фактического listen-порта;

- диагностике SSH-аутентификации;

- `Too many authentication failures` при большом количестве ключей в SSH-agent.

- Добавлены вопросы по работе с панелями управления:

- различие PHP сайта и PHP CLI в FASTPANEL;

- разделение nginx frontend и Apache backend в BitrixVM;

- ручные изменения конфигурации, которые перезаписываются ISPmanager.

- Расширен Web-блок.

- Добавлены сценарии по:

- определению источника 403 / 404 / 500 между nginx и backend;

- работе virtual host, `Host` и SNI;

- корректной проверке HTTPS-сайта на новом IP до переключения DNS;

- работе reverse proxy;

- передаче `Host` и `X-Forwarded-Proto`;

- различию внешнего HTTPS и внутреннего HTTP между nginx и backend;

- определению фактического PHP-FPM socket / TCP endpoint;

- ситуации, когда сайт работает на неожиданной версии PHP;

- исчерпанию `max_connections` MySQL;

- различию `Threads_connected` и `Threads_running`;

- периодическим 502 после завершения PHP-FPM worker через OOM Killer;

- восстановлению причинной цепочки по событиям nginx, PHP-FPM и kernel.

- Расширен сетевой блок.

- Добавлены сценарии по:

- различию ICMP-доступности и доступности реального TCP-сервиса;

- диагностике маршрута через ICMP MTR и TCP MTR;

- анализу потерь на промежуточных узлах;

- определению уровня проблемы по `tcpdump`;

- ситуации, когда SYN доходит до сервера, SYN-ACK отправляется, но не возвращается клиенту;

- локализации проблемы на обратном сетевом пути без преждевременного вывода о конкретном операторе или фильтрации;

- асимметричной производительности через `iperf3`;

- диагностике IPv6;

- проблемам Path MTU;

- ситуации, когда TCP/443 доступен и MTR выглядит нормально, но крупные TCP-сегменты не проходят.

### Форматы вопросов

- Продолжено использование разных механик вопросов, чтобы тест не превращался в однотипный multiple choice.
...
- выбор строк из логов или конфигурации.

- Для вопросов с несколькими правильными ответами убран фиксированный шаблон «всегда два правильных»:

- чаще используется 2–3 правильных;

- количество зависит от конкретной диагностической ситуации.

- Для вопросов типа «выбор строк» учтено, что строки не перемешиваются:

- правильные варианты распределяются среди неправильных;

- порядок реального лога или конфигурации сохраняется, если он имеет диагностическое значение.

- Сложность вопросов пересмотрена:

- вопросы, где необходимо связать несколько источников данных, переводятся в «Сложные»;

- для таких сценариев время увеличено до 240 секунд;

- простые диагностические вопросы остаются на уровне 150–180 секунд.

### Подход к диагностическим вопросам

- Усилен акцент на причинно-следственную диагностику вместо знания отдельных команд.

- В вопросах сотруднику приходится сопоставлять данные из нескольких источников, например:
...
- nginx / PHP-FPM / application logs.

- Добавлены ловушки на распространённые ошибочные выводы:

- высокий Load Average не обязательно означает загрузку CPU;

- низкий `free` не обязательно означает нехватку RAM;

- работающий systemd-сервис не гарантирует доступность нужного порта;

- открытый TCP-порт не гарантирует работоспособность следующего уровня;

- HTTP-код, который клиент получил через nginx, не обязательно сформирован nginx;

- успешный `nginx -t` не подтверждает правильность прикладной маршрутизации;

- наличие PHP-FPM сервиса не означает, что конкретный сайт использует именно его;

- нормальный MTR не исключает проблему передачи крупных пакетов;

- временная близость событий в логах сама по себе не доказывает причинную связь.

- В сетевых вопросах явно указывается, где выполняется каждая команда:
...
- Зафиксирован подход не делать вывод о проблеме конкретного провайдера, ТСПУ или инфраструктуры только по промежуточным ICMP-потерям.

### Интерфейс модуля оценки

- Исправлена читаемость сообщения об успешном сохранении вопроса:

- используется более контрастный текст;

- success-сообщение приведено к общей цветовой схеме assessment.

- Исправлено русское склонение количества семейств:
...
- Для склонений добавлен универсальный template filter, который в дальнейшем можно использовать для вопросов, попыток и других счётчиков.

- Продолжена доработка редактора вопросов и диагностических данных.

- Для диагностических данных используются отдельные структурированные блоки:

- текстовое пояснение;

- код / лог / конфигурация.

- Это позволяет не хранить весь диагностический сценарий одним большим текстовым полотном и визуально отделять команды и логи от пояснений.

- Для строк диагностического задания выявлена проблема с многострочным содержимым:

- обычный `input` объединял команду и её вывод в одну строку;

- принято решение использовать многострочное поле для одного выбираемого элемента;

- переносы внутри одной диагностической строки должны сохраняться и в редакторе, и при прохождении теста.

### Качество банка

- При создании новых вопросов отдельно проверяется, не дублирует ли вопрос уже существующий навык или семейство.

- Близкие по теме, но разные диагностические ситуации разделяются на разные семейства.

- Варианты одной и той же ситуации остаются внутри одного семейства.

- Сокращено количество вопросов, где правильный ответ можно определить только по длине или формулировке варианта.

- Дистракторы делаются технически правдоподобными.

- В комментариях наставнику отдельно объясняется:

- почему правильный вывод следует из данных;

- почему альтернативные направления диагностики преждевременны;

- какой следующий шаг логичен после определения причины.


### Ближайший план

- Продолжить расширение банка Ticket Sandbox.
- Добавить новые сетевые диагностические кейсы.
- Подготовить сценарий, связанный с ТСПУ:
  - origin и сервис на сервере работают;
  - локальные проверки успешны;
  - внешняя доступность отличается;
  - задача стажёра — определить уровень проблемы и не пытаться чинить исправный nginx.
- Продумать ещё несколько кейсов, где основной навык — локализация проблемы, а не выполнение заранее очевидной команды.
- Продолжить собирать обратную связь после реального использования тренажёра сотрудниками.

- Продолжить добавлять вторые и третьи диагностические ситуации по уже покрытым техническим навыкам.

- Следить за балансом форматов вопросов, сложности и времени на выполнение.

- Довести редактор многострочных вариантов для типа «Выбор строк».
...
## Неделя 20 — права файлов, светлая тема и прототип AI-проверки ответов

### Исправление прав файлов training_tasks

На staging была обнаружена проблема с правами файлов внутри некоторых учебных заданий.

В частности, PHP-файлы в заданиях могли попадать в Docker image с правами:

600 root:root

Из-за этого PHP-FPM не мог прочитать файлы, а задания возвращали 403 Access denied.

Проверено, что проблема находилась не в Dockerfile и не в PHP-FPM: в рабочем каталоге staging права файлов уже отличались от ожидаемых Git-состоянием.

Для восстановления прав добавлена нормализация файлов на основе Git index:
...
deploy/normalize_training_task_permissions.sh

Скрипт:

- восстанавливает ожидаемые права для Git-tracked файлов training_tasks;
- нормализует права директорий;
- проверяет, что необходимые файлы доступны для чтения внутри task-container.

Локально воспроизведён сценарий с chmod 600 и подтверждено восстановление прав скриптом.

### Нормализация прав в CI/CD

В staging deployment добавлен отдельный шаг:

Normalize training task permissions

Он выполняется после получения нужного commit и до сборки Docker images.

Теперь некорректные права файлов в staging worktree не должны попадать в новые task images.

После изменения staging deployment прошёл успешно.


### Светлая и тёмная тема

Для Ticket Sandbox и Trainee Diary добавлена поддержка светлой темы.

Используется общий механизм через:
...
- восстановление темы при следующем открытии;
- синхронизация темы между вкладками;
- переключение иконки солнце / луна;
- динамический aria-label кнопки.

Тёмная тема остаётся темой по умолчанию.

Assessment пока не переводился на новый механизм и остался без изменений.

### Адаптация интерфейсов под светлую тему

Дополнительно поправлены отдельные элементы Trainee Diary:

- dashboard;
- карточка стажёра;
- pre-adaptation users;
- цвета этапов;
- границы и фоновые элементы.

Терминал намеренно остаётся в тёмном оформлении независимо от основной темы.


### Прототип AI-проверки ответов стажёров

Начата работа над AI-проверкой текстовых ответов стажёров.

Для прототипа создан отдельный AI-agent Fair Woodpecker в Timeweb Cloud AI Agents.

Определены основные критерии проверки:
...
Используются три уровня результата:
...
- critical — ответ требует доработки;
- minor — ответ можно принять, но дать рекомендацию;
- ok — замечаний нет.

### Структурированный AI-response

Ответ AI переведён в JSON-формат:
...
Для проверки полноты отдельно добавлен:

"missing_facts": []

Это позволяет получать от модели не свободный текст, а конкретный список обязательной информации, которую стажёр пропустил.

### Проверка AI-критериев

На тестовых сценариях проверены:

- корректный короткий ответ;
- корректный подробный ответ;
- неверная причина проблемы;
- выдуманные действия;
- отсутствие приветствия;
- отсутствие обращения по имени;
- приветствие, привязанное ко времени суток;
- излишне технический язык;
- плохая пунктуация и структура;
- слишком расплывчатое описание причины и решения;
- отсутствие обязательного действия клиента;
- отсутствие ответа по существу.

Промпт постепенно скорректирован так, чтобы разные критерии не дублировали замечания друг друга.

Текущую версию можно считать v1 для дальнейшей интеграции.


### Проверка Timeweb AI Agents API

Проверен доступ к Fair Woodpecker через OpenAI-compatible API.

Подтверждена цепочка:

Python
  ↓
Timeweb AI Agents API
  ↓
Fair Woodpecker
  ↓
JSON response
...
- вызов через curl;
- вызов через Python;
- применение системной инструкции агента через API;
- корректный разбор вложенного JSON-ответа.

Тестовый Python-клиент успешно получил структурированный результат проверки ответа стажёра.

Интеграция непосредственно в Ticket Sandbox пока не выполнялась.


### Планируемая архитектура AI-review

На текущем этапе принято решение не давать AI автоматически принимать финальное решение по попытке.

Первый вариант интеграции:

стажёр отправляет ответ
        ↓
Celery
        ↓
AI reviewer
...
результат сохраняется
        ↓
наставник видит AI-анализ
        ↓
финальное решение принимает наставник

Это позволит сначала сравнить решения AI и наставников на реальных учебных ответах.

В дальнейшем можно собирать статистику:
...
Для AI также планируется хранить отдельную историю проверок, чтобы повторная отправка одного TaskAttempt после needs_revision не затирала предыдущий результат.


### Архитектурная подготовка AI-контекста

При анализе текущего Ticket Sandbox выявлено, что для качественной AI-проверки недостаточно только текста тикета и результата check.sh.

AI должен получать скрытые эталонные факты задания:
...
Планируется добавить их в task.json отдельным блоком:
...
Этот блок не будет показываться стажёру и будет использоваться только для проверки его ответа.


### Текущее состояние

- проблема с правами training_tasks исправлена и защищена на уровне deployment;
- светлая тема Sandbox и Trainee Diary завершена и задеплоена;
- AI reviewer подготовлен и протестирован отдельно от приложения;
- API-доступ к AI-agent подтверждён;
- интеграция AI в Django / Celery ещё не начата.

### Следующие задачи

- добавить ai_review_context в модель задания и task.json;
- научить sync_training_tasks синхронизировать AI-контекст;
- добавить Django service для вызова Timeweb AI Agents;
- добавить модель истории AI-проверок;
- выполнять AI-review через Celery;
- показать результат AI наставнику рядом с попыткой;
- собрать первые данные по совпадению решений AI и наставника;
- только после этого рассматривать автоматический needs_revision.